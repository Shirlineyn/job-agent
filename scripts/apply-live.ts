...
async function retry<T>(fn: () => Promise<T>, tries = 3): Promise<T> {
  ...
  for (let i = 0; i < tries; i++) {
    try { return await fn(); }
      ...
      last = e;
      if (!/40[38]|429|5\d\d|timeout|timed out|network|fetch failed|ECONN/i.test(String(e))) throw e;
...
async function genLetter(db: Database, cfg: Config, resume: string, v: VacancyRow, runId: number): Promise<string> {
  const ctx = { db, runId, vacancyId: v.id };
  let research = "";
  try { research = await retry(() => researchCompany(ctx, callPerplexity, cfg, v.employer_id ?? v.employer_name, v.employer_name)); }
  catch (e) { console.log("  рисёрч упал (", String(e).slice(0, 50), ") — письмо без справки."); }
  const text = (JSON.parse(v.raw_json ?? "{}") as { text?: string }).text ?? v.title;
  const score = JSON.parse(v.score_reasons ?? "{}");
  return retry(() => writeLetter(ctx, callClaude, cfg, { resume, vacancyText: text, research, score }));
}
...
async function main() {
  if (cmd !== "letters" && cmd !== "submit") { console.log("usage: apply-live.ts letters|submit <id> [<id> ...]"); process.exit(1); }
  if (ids.length === 0) { console.log("укажи id вакансий"); process.exit(1); }
  ...
  console.log(`⚠ LIVE — реальные отклики. Вакансий: ${ids.length}. dailyLimit=${cfg.dailyLimit}, уже сегодня=${repo.appliedToday(db)}.`);
  const runId = repo.startRun(db, "manual", "live");
  const browser = new HhBrowser();
  await browser.launch(join(homedir(), ".hh-agent", "profile"));
  let applied = 0, stop = "completed";
  try {
    for (const id of ids) {
      if (repo.appliedToday(db) >= cfg.dailyLimit) { console.log("дневной лимит достигнут — стоп."); stop = "daily_limit"; break; }
      const v = repo.getVacancy(db, id);
      if (!v) { console.log(`[${id}] нет в БД`); continue; }
      ...
      if (!letter) {
        ...
        try { letter = await genLetter(db, cfg, resume, v, runId); repo.setStatus(db, id, "queued", { letter }); }
    ...
    else if (e instanceof LoggedOut) { stop = "logged_out"; console.log("РАЗЛОГИН — стоп. Залогинься (scripts/login.ts)."); }
    else { stop = "error"; console.log("ERROR:", e); }
  }
