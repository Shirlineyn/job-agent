...
if (!url || !/hh\.ru\/vacancy\/\d+/.test(url)) {
  console.error(
    'Укажи URL вакансии: npx tsx scripts/probe-apply.ts "https://hh.ru/vacancy/123456"',
  );
  process.exit(2);
}
...
async function snapshot(page: Page) {
  ...
  const [textarea, textInput, radio, checkbox, select, letterInput, letterToggle, submit] =
    await Promise.all([
      c("textarea"),
      c("input[type='text'], input:not([type])"),
      c("input[type='radio']"),
  ...
  const dataQaRaw = await page.$$eval("[data-qa]", (els) =>
    els.map((e) => e.getAttribute("data-qa") || ""),
  );
  const dataQa = [
    ...new Set(
      dataQaRaw.filter((v) =>
        /response|letter|task|question|test|cover|submit|popup|relocat|region|area|city|confirm/i.test(
          v,
        ),
      ),
    ),
  ].sort();
  const labelsRaw = await page.$$eval(
    "label, [data-qa*='question'], [data-qa*='task'], fieldset legend",
    (els) => els.map((e) => (e.textContent || "").replace(/\s+/g, " ").trim()),
  );
  const labels = [...new Set(labelsRaw.filter((t) => t.length > 3 && t.length < 220))].slice(0, 40);
  const btnsRaw = await page.$$eval("button, [role='button'], a[data-qa]", (els) =>
    els.map(
      (e) =>
  ...
  return {
    ...
    letterToggle,
    submit,
    alreadyApplied: /Вы откликнулись|Отклик доставлен|Резюме отправлено|Вы уже откликались/i.test(
      bodyText,
    ),
    dataQa,
    buttons,
...
async function main() {
  ...
  console.log(
    "[probe-apply] isLoggedOut:",
    await browser.isLoggedOut(),
    "| isCaptcha:",
    await browser.isCaptcha(),
  );

  const respond = page.locator('[data-qa="vacancy-response-link-top"]').first();
  if ((await respond.count()) === 0) {
    console.log("[probe-apply] нет кнопки отклика — уже откликались/недоступно. Выхожу.");
    await browser.close();
    return;
  }
  ...
  for (let step = 1; step <= 4; step++) {
    ...
    if (s.alreadyApplied) {
      console.log("⚠️ отклик, похоже, уже отправлен.");
      break;
    }
    if (reachedForm(s)) {
      console.log(
        `[probe-apply] дошли до формы/анкеты (шаг ${step}) — снимаю и останавливаюсь ДО submit.`,
      );
      break;
    }
    ...
    if ((await relocate.count()) > 0) {
      console.log("[probe-apply] relocation confirm найден — жму.");
      await relocate.click();
      continue;
    }
    const advance = page
      .locator('button:visible, [role="button"]:visible')
      .filter({ hasText: /продолжить|далее|подтвердить|выбрать|откликнуться/i });
    const n = await advance.count();
    let clicked = false;
    for (let i = 0; i < n; i++) {
      const el = advance.nth(i);
      const dq = await el.getAttribute("data-qa");
      if (dq?.includes("submit-popup")) continue; // финальный submit — не жмём
      console.log(
        `[probe-apply] жму advance: «${(await el.textContent())?.trim()}» [${dq ?? "no-dqa"}]`,
      );
      await el.click();
      clicked = true;
      break;
    ...
    if (!clicked) {
      console.log("[probe-apply] нет кнопки прохода дальше — стоп на текущем шаге.");
      break;
    }
  }
...
main().catch(async (err) => {
  console.error("[probe-apply] ERROR:", err);
  process.exit(1);
});
