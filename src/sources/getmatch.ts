...
import { parseWorkFormat, fetchTextFromRawJson } from "./shared.js";
...
interface Offer {
  ...
  location_items: { format?: string }[];
  published_at: string | null;
  url?: string | null;
}
interface OffersResp {
  meta: { total: number };
  offers: Offer[];
}
...
function fmt(items: { format?: string }[]): WorkFormat {
  return parseWorkFormat(items.map((i) => i.format ?? "").join(" "));
}
  ...
  return {
    ...
    async search(keywords: string[], _cfg: Config): Promise<VacancyInsert[]> {
      ...
      for (let offset = 0; ; offset += 100) {
        const resp = await getJson<OffersResp>(
          f,
          `https://getmatch.ru/api/offers?limit=100&offset=${offset}`,
        );
        if (!Array.isArray(resp.offers))
          throw new Error("getmatch: неожиданная схема (нет offers)");
        for (const o of resp.offers) {
          ...
          out.push({
            id: `getmatch:${o.id}`,
            url: absUrl(o.url, o.id),
            title: o.position,
            employer_id: `getmatch:${o.company.name.toLowerCase().trim()}`,
            employer_name: o.company.name,
            salary_from: o.salary_display_from,
            salary_to: o.salary_display_to,
            currency: o.salary_currency === "RUB" ? "RUR" : o.salary_currency,
            work_format: fmt(o.location_items ?? []),
            experience: null,
    ...
    fetchText(v: VacancyRow): Promise<string> {
      return Promise.resolve(fetchTextFromRawJson(v, "getmatch"));
    },
  };
}
