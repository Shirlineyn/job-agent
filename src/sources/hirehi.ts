...
export function parseSalary(s: string | null | undefined): { from: number | null; to: number | null } {
  ...
  const hasFrom = /от/.test(s);
  const hasTo = /до/.test(s);
  if (hasFrom && hasTo) return { from: nums[0], to: nums[1] ?? null };
  if (hasFrom) return { from: nums[0], to: null };
  if (hasTo) return { from: null, to: nums[0] };
  // "~ N ₽" или голое число — единственная оценка, трактуем как одновременно from и to
  return { from: nums[0], to: nums[1] ?? nums[0] ?? null };
}
      ...
      for (const kw of keywords) {
        for (let page = 1; page <= PAGES_PER_KEYWORD; page++) {
          ...
          for (const j of resp.jobs) {
            ...
            out.push({
              id, url: `https://hirehi.ru/${j.category ?? "vacancy"}/vacancy-${j.id}`, title: j.title,
              ...
              published_at: j.created_at ?? null, raw_json: JSON.stringify(j), source: "hirehi",
            });
          }
