...
export function applyHardFilters(
  ...
  if (blacklist.some((p) => name.includes(p.toLowerCase()) || p === v.employer_id))
  ...
  if (!f.workFormats.includes(v.work_format ?? "unknown"))
    return { pass: false, reason: "work_format" };
  if (v.experience && !f.maxExperience.includes(v.experience))
