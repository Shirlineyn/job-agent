...
export class HhBrowser {
  ...
  isAlive(): boolean {
    return !this.closed && this.ctx !== undefined;
  }

  async isCaptcha(): Promise<boolean> {
    return (
      this.page.url().includes("captcha") || (await this.page.locator(SEL.captcha).count()) > 0
    );
  }
  ...
  async searchVacancies(
    query: string,
    area: number,
    orderBy: "publication_time" | "relevance" | "default" = "publication_time",
  ): Promise<VacancyInsert[]> {
            ...
            return {
              ...
              work_format: /удал[её]нн/i.test(c.textContent ?? "")
                ...
                : /гибрид/i.test(c.textContent ?? "")
                  ...
                  : "unknown",
              experience: null,
              published_at: null, // уточняются на странице вакансии
  ...
  async hasQuestionnaire(): Promise<boolean> {
    return (
      (await this.page
        .locator('[data-qa="employer-asking-for-test"], [data-qa="task-question"]')
        .count()) > 0
    );
  }
  ...
  async extractQuestionnaire(): Promise<QuestionnaireItem[]> {
        ...
        els.map((e) =>
          e.matches('[data-qa="task-question"]')
            ...
            : e.tagName === "TEXTAREA"
              ...
              : {
                  kind: (e as HTMLInputElement).type === "checkbox" ? "checkbox" : "radio",
    ...
    for (const it of items) {
      if (it.kind === "q") {
        cur = { name: "", question: it.text, options: [], textName: null, multi: false };
        questions.push(cur);
        continue;
      }
      ...
      if (it.kind === "radio") {
        cur.name = it.name;
        cur.options.push({ value: it.value, text: it.text });
      } else if (it.kind === "checkbox") {
        cur.name = it.name;
        cur.multi = true;
        cur.options.push({ value: it.value, text: it.text });
      } else if (it.kind === "textarea") {
        cur.textName = it.name;
      }
    }
    return questions;
  }
    ...
    for (const a of answers) {
      ...
      if (a.type === "option") {
        ...
        for (const val of vals)
          await this.page.locator(`input[name="${q.name}"][value="${val}"]`).check({ force: true });
        ...
        if (q.options.length > 0) {
          ...
          if (custom)
            await this.page
              .locator(`input[name="${q.name}"][value="${custom.value}"]`)
              .check({ force: true });
          await sleep(300, 700);
        }
        // textarea точечно по имени task_NNN_text (надёжнее, чем индекс среди всех textarea)
        const ta = q.textName
          ? this.page.locator(`textarea[name="${q.textName}"]`)
