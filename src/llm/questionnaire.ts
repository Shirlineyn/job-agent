...
import type { Config, Candidate } from "../config.js";

export const QUESTIONNAIRE_SYSTEM = (
  c: Candidate,
) => `Ты — ИИ-агент, отвечающий на анкету работодателя от лица кандидата ${c.nameGenitive}.
...
- Вопрос с ЧЕКБОКСАМИ (помечен «МОЖНО НЕСКОЛЬКО»): type:"option", верни ВСЕ подходящие value в поле "values" (массив), честно из резюме. Напр. город проживания — ${c.city}.
...
export async function answerQuestionnaire(
  ...
  const body = questions
    .map((q, i) =>
      q.options.length > 0
        ...
        : `Вопрос ${i} (ОТКРЫТЫЙ, свободный текст): ${q.question}`,
    )
    .join("\n\n");
  ...
  const raw = await claude(ctx, {
    model: cfg.anthropicModel,
    system: [QUESTIONNAIRE_SYSTEM(cfg.candidate), `<резюме>\n${resume}\n</резюме>`],
    user,
    maxTokens: 1024,
    purpose: "letter",
  });
