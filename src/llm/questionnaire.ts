...
Для каждого вопроса:
- если ОДИН из предложенных вариантов точно соответствует правде из резюме — выбери его (type:"option", value этого варианта);
- если ни один вариант не отражает правду точно (например, опыт частичный, смежный или нюансированный) — используй "Свой вариант" (type:"custom") и напиши короткий честный ответ в 1 фразу.
...
[{"i":0,"type":"option","value":"<value>"} | {"i":0,"type":"custom","text":"<честный краткий ответ>"}]`;
...
export type QuestionnaireItem = { name: string; question: string; options: { value: string; text: string }[] };
...
export async function answerQuestionnaire(
  ...
  const body = questions.map((q, i) =>
    `Вопрос ${i}: ${q.question}\nВарианты: ${q.options.map(o => `[value=${o.value}] ${o.text}`).join(" | ")}`).join("\n\n");
