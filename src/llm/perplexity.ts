...
export async function callPerplexity(ctx: LlmLogCtx, opts: { model: string; prompt: string; purpose: "research" | "email_search" }): Promise<string> {
  ...
  try {
    ...
    logCall(ctx, { provider: "perplexity", purpose: opts.purpose, model: opts.model, request: req, response: res, error: null,
      inputTokens: res.usage?.prompt_tokens ?? null, outputTokens: res.usage?.completion_tokens ?? null, latencyMs: Date.now() - t0 });
    return res.choices[0]?.message.content ?? "";
  } catch (e) {
    logCall(ctx, { provider: "perplexity", purpose: opts.purpose, model: opts.model, request: req, response: null,
