...
export async function callClaude(ctx: LlmLogCtx, opts: {
  model: string; system: string | string[]; user: string; maxTokens: number; purpose: "scoring" | "letter";
}): Promise<string> {
  ...
  const system = texts.map((text, i) => i === texts.length - 1
  ...
  try {
    ...
    logCall(ctx, { provider: "anthropic", purpose: opts.purpose, model: opts.model, request: req, response: res,
      error: null, inputTokens: res.usage.input_tokens, outputTokens: res.usage.output_tokens,
      cacheCreationTokens: res.usage.cache_creation_input_tokens, cacheReadTokens: res.usage.cache_read_input_tokens,
