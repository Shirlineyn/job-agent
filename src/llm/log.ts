...
export function cost(
  ...
  return (
    ((inTok + 1.25 * cacheWriteTok + 0.1 * cacheReadTok) * p.inUsd + outTok * p.outUsd) / 1_000_000
  );
}
...
export function logCall(
  ...
  repo.insertLlmCall(ctx.db, {
    vacancy_id: ctx.vacancyId,
    run_id: ctx.runId,
    provider: row.provider,
    purpose: row.purpose,
    model: row.model,
    request: JSON.stringify(row.request),
    response: row.response ? JSON.stringify(row.response) : null,
    error: row.error,
