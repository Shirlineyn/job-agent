...
export function upsertVacancy(db: Database, v: VacancyInsert): boolean {
  ...
  const run = db.transaction((row: VacancyInsert & { key: string }) => {
    if (row.employer_id != null) {
      db.prepare(
        `INSERT OR IGNORE INTO companies (employer_id, name) VALUES (@employer_id, @employer_name)`,
      ).run(row);
    }
...
export function setStatus(
  ...
  const cols = Object.keys(extra)
    ...
    .join(",");
  db.prepare(
    `UPDATE vacancies SET status=@status, updated_at=datetime('now')${cols ? "," + cols : ""} WHERE id=@id`,
  ).run({ id, status, ...extra });
}

export function getByStatus(db: Database, status: VacancyStatus): VacancyRow[] {
  return db
    .prepare(`SELECT * FROM vacancies WHERE status=? ORDER BY discovered_at`)
...
export function appliedToday(db: Database): number {
  ...
  const r = db
    .prepare(
      `SELECT COUNT(*) n FROM vacancies WHERE status='applied' AND date(applied_at,'localtime')=date('now','localtime')`,
    )
    .get() as { n: number };
  return r.n;
}
...
export function insertLlmCall(db: Database, c: LlmCallInsert): void {
  db.prepare(
    `INSERT INTO llm_calls (vacancy_id,run_id,provider,purpose,model,request,response,error,input_tokens,output_tokens,cache_creation_tokens,cache_read_tokens,cost_usd,latency_ms)
    VALUES (@vacancy_id,@run_id,@provider,@purpose,@model,@request,@response,@error,@input_tokens,@output_tokens,@cache_creation_tokens,@cache_read_tokens,@cost_usd,@latency_ms)`,
  ).run(c);
}
...
export function startRun(
  ...
  return Number(
    db.prepare(`INSERT INTO runs (trigger,mode) VALUES (?,?)`).run(trigger, mode).lastInsertRowid,
  );
}
...
export function getBlacklist(db: Database): string[] {
  return (db.prepare(`SELECT pattern FROM blacklist`).all() as { pattern: string }[]).map(
    (r) => r.pattern,
  );
}
export function addBlacklist(db: Database, pattern: string, reason?: string): void {
  db.prepare(`INSERT OR IGNORE INTO blacklist (pattern,reason) VALUES (?,?)`).run(
    pattern,
    reason ?? null,
  );
}
...
export function getCompanyResearch(
  db: Database,
  employerId: string,
): { research: string; researchedAt: string } | null {
  const r = db
    .prepare(
      `SELECT research, researched_at FROM companies WHERE employer_id=? AND research IS NOT NULL`,
    )
    .get(employerId) as { research: string; researched_at: string } | undefined;
...
export function saveCompanyResearch(
  ...
  db.prepare(
    `INSERT INTO companies (employer_id,name,research,researched_at) VALUES (?,?,?,datetime('now'))
    ON CONFLICT(employer_id) DO UPDATE SET research=excluded.research, researched_at=excluded.researched_at`,
  ).run(employerId, name, research);
}
...
export function getCompanyEmail(
  ...
  const r = db
    .prepare(
      `SELECT contact_email, email_status, email_checked_at FROM companies
    WHERE employer_id=? AND email_status IS NOT NULL`,
    )
    .get(employerId) as
    | {
    ...
    | undefined;
  return r
    ? { email: r.contact_email, status: r.email_status, checkedAt: r.email_checked_at }
    : null;
}

export function saveCompanyEmail(
  db: Database,
  employerId: string,
  name: string,
  email: string | null,
  source: "source_payload" | "perplexity" | null,
): void {
  db.prepare(
    `INSERT INTO companies (employer_id, name, contact_email, email_status, email_checked_at, email_source)
    VALUES (?,?,?,?,datetime('now'),?)
...
export function insertEmailDraft(db: Database, e: EmailInsert): boolean {
  const r = db
    .prepare(
      `INSERT OR IGNORE INTO emails (vacancy_id,to_email,subject,body)
    VALUES (@vacancy_id,@to_email,@subject,@body)`,
    )
    .run(e);
  return r.changes > 0;
}
...
export function updateEmailDraft(
  ...
  db.prepare(
    `UPDATE emails SET subject=COALESCE(@subject,subject), body=COALESCE(@body,body)
...
export function getUndraftedEmails(db: Database): EmailRow[] {
  return db
    .prepare(
      `SELECT * FROM emails WHERE status='draft' AND gmail_drafted_at IS NULL ORDER BY created_at`,
    )
    .all() as EmailRow[];
}
...
export function emailsSentToday(db: Database): number {
  const r = db
    .prepare(
      `SELECT COUNT(*) n FROM emails WHERE status='sent'
...
export function report(db: Database, date: string): unknown {
  return {
    ...
    applied: db
      .prepare(
        `SELECT id,title,employer_name,score,applied_at FROM vacancies WHERE date(applied_at)=?`,
      )
      .all(date),
    costUsd: (
      db
        .prepare(`SELECT COALESCE(SUM(cost_usd),0) c FROM llm_calls WHERE date(created_at)=?`)
