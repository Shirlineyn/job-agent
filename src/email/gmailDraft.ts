...
export type GmailDraft = { id: number; to: string; subject: string; body: string };
...
export async function appendToGmailDrafts(
  cfg: Config, drafts: GmailDraft[], opts: AppendOpts = {},
): Promise<number> {
  ...
  const client = (opts.makeClient ?? defaultFactory)(
    { host: "imap.gmail.com", port: 993, secure: true, auth: { user: cfg.smtp.user, pass } });
