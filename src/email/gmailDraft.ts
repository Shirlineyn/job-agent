...
export type GmailDraft = { to: string; subject: string; body: string };
...
const defaultFactory: ImapClientFactory = (opts) => new ImapFlow({ ...opts, logger: false }) as unknown as ImapClientLike;
...
export async function appendToGmailDrafts(
  cfg: Config, drafts: GmailDraft[], makeClient: ImapClientFactory = defaultFactory,
): Promise<number> {
  if (drafts.length === 0) return 0;
  const pass = process.env.SMTP_PASSWORD;
  if (!pass) throw new Error("SMTP_PASSWORD не задан в env — черновики в Gmail недоступны");

  const client = makeClient({ host: "imap.gmail.com", port: 993, secure: true, auth: { user: cfg.smtp.user, pass } });
  ...
  try {
    ...
    for (const d of drafts) {
      const mime = await new MailComposer({
        ...
        to: d.to, subject: d.subject, text: d.body,
        attachments: cfg.resumePdfPath ? [{ filename: basename(cfg.resumePdfPath), path: cfg.resumePdfPath }] : [],
      }).compile().build();
