...
const cand = CandidateSchema.parse({});
const wl = ["tedo.ru", "github.com"];
const check = (text: string) => validateLetter(text, cand, wl);
...
describe("validateLetter", () => {
  it("accepts a well-formed letter", () => {
    expect(check(ok).ok).toBe(true);
  });
  it("rejects too short", () => {
    expect(check("Привет, возьмите меня. Доронин").ok).toBe(false);
  });
  it("rejects foreign urls", () => {
    expect(check(ok + " http://evil.example.com").ok).toBe(false);
  });
  it("allows whitelisted urls", () => {
    expect(check(ok + " https://tedo.ru/insights/gartner-hype-cycle").ok).toBe(true);
  });
  it("rejects lookalike domains", () => {
    expect(check(ok + " http://eviltedo.ru/x").ok).toBe(false);
  });
});
...
describe("EMAIL_LETTER_SYSTEM_V1", () => {
  ...
  it("вставляет ссылку на репозиторий, когда repoUrl задан", () => {
    const p = EMAIL_LETTER_SYSTEM_V1(cand, url);
  ...
  it("без repoUrl не добавляет URL и запрещает ссылки", () => {
    const p = EMAIL_LETTER_SYSTEM_V1(cand, null);
  ...
  it("просит маркированный список пересечений", () => {
    expect(EMAIL_LETTER_SYSTEM_V1(cand, null)).toMatch(/маркированным списком/);
  });
});
