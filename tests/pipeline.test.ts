...
function deps(overrides: Partial<Deps> = {}): Deps {
  ...
  const cfg = {
    ...loadConfig(mkdtempSync(join(tmpdir(), "hh-"))),
    applyPauseMs: [0, 0] as [number, number],
  };
  return {
    db,
    cfg,
    ...
    pplx: vi.fn(async () => "справка о компании"),
    notify: vi.fn(),
    sources: [],
    ...overrides,
  };
...
describe("runSession", () => {
  ...
  it("skips below-threshold vacancies", async () => {
    const d = deps({
      claude: vi.fn(
        async () =>
          '{"score":30,"reasons":[],"red_flags":["галера"],"salary_match":"unknown","seniority_match":"match"}',
      ),
    });
    const s = await runSession(d, "manual", "live");
    expect(s.applied).toBe(0);
  ...
  it("keeps vacancy queued on transient research error (retry next session)", async () => {
    const d = deps({
      pplx: vi.fn(async () => {
        throw Object.assign(new Error("perplexity 403 forbidden"), { status: 403 });
      }),
    });
    const s = await runSession(d, "manual", "live");
    expect(s.errors).toBeGreaterThan(0);
